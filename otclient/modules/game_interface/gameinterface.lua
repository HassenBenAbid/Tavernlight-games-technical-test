...
function jumpClick(button)
  local currentPosition = button:getPosition() --Get the current position of the button.

  --We make sure that this button has a parent window.
  local parent = button:getParent() 
  if not parent then
    print("Error: jump button not within a parent window.")
    return 
  end

  --We make sure that this button parent was actually attached to a root widget.
  local rootWidget  = g_ui.getRootWidget()
  if not rootWidget then 
    print("Error: jump window wans't attached to the root widget.")
    return
  end

  local newPosition = { x = currentPosition.x, y = currentPosition.y } --The new position that will be assigned to the button.

  --If the button is closer to the top then we move the button down.
  if math.abs(currentPosition.y - rootWidget:getHeight()) > currentPosition.y then
    newPosition.y = newPosition.y + parent:getHeight() * 0.5 - 50 -- move up by half the size of the window
  else
    newPosition.y = newPosition.y - parent:getHeight() * 0.5 - 50 -- move down by half the size of the window
  end

  --Removing this button from its parent and then adding it again make sure that the position is updated correctly on the screen.
...
function initJump()
  g_ui.importStyle('styles/jumpwindow') --We start by importing the jump window style.

  --We then create the jump window from the imported style.
  jumpWindow = g_ui.createWidget('JumpWindow', rootWidget)

  --Get the button by its ID and add the onClick event.
  jumpButton = jumpWindow:getChildById('jumpButton')
  jumpButton.onClick = jumpClick

  --Finally we start the movement tick.
  jumpTimer = scheduleEvent(moveJumpButton, 200)
end

function moveJumpButton()
  --We move the button horizontally.
  local newX = jumpButton:getPosition().x - 12

  --We make sure that this button has a parent window.
  local parent = jumpButton:getParent() 
  if not parent then
    print("Error: jump button not within a parent window.")
    return 
  end

  --Check if the button has reached the left edge of the window and when it does call jump click.
  if newX <= parent:getPosition().x then
      jumpClick(jumpButton)
      print("jump move")
      newX = parent:getPosition().x + parent:getWidth() - jumpButton:getWidth() - 20 --We make sure that the whole button is inside the window.
  end

  -- Set the new button position
  parent:removeChild(jumpButton)
  jumpButton:setPosition({ x = newX, y = jumpButton:getPosition().y })
  parent:addChild(jumpButton)

  jumpTimer = scheduleEvent(moveJumpButton, 200) --Finally we schedule another event so that the button will keep on moving.
end

--This will take care of destroying the timer and the window when it is time to close it.
function terminateJump()
  --Destroy the window when the module terminates.
  if jumpwindow then
    jumpWindow:destroy()
  end

  if jumpTimer then 
    removeEvent(jumpTimer)
    jumpTimer = nil
  end
end
